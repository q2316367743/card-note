            ...
            <a-list-item v-for="key in keys" :key="key" :span="24">
                ...
                <template #actions>
                    <a-button-group type="text">
                        ...
                        <a-popconfirm content="确认删除此附件，如果文章引用了附件，则无法显示" ok-text="删除"
                                      @ok="onRemove(key)">
...
import {listRecordByAsync, removeAttachmentAsync} from "@/utils/utools/DbStorageUtil";
import {ATTACHMENT_PREFIX} from "@/entity/Role";
import Constant from "@/global/Constant";
import {copy} from "@/utils/lang/BrowserUtil";
import MessageUtil from "@/utils/modal/MessageUtil";
import {buildUrlFromUtools, transformImgUrl} from "@/plugin/image";
...
function onRemove(url: string) {
    ...
    removeAttachmentAsync(buildUrlFromUtools(url))
        .then(() => {
            MessageUtil.success("删除附件成功");
            init();
        })
        .catch(e => MessageUtil.error("删除附件失败", e))
